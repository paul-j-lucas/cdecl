...
c_ast_t const* c_ast_find_param_named( c_ast_t const *func_ast,
  ...
  c_sname_t const sname = C_SNAME_LIT( name );
...
void c_ast_op_params_min_max( c_ast_t const *ast, unsigned *rv_params_min,
  ...
  assert( ast != NULL );
  assert( ast->kind == K_OPERATOR );
  assert( rv_params_min != NULL );
  assert( rv_params_max != NULL );
  ...
  switch ( c_ast_op_overload( ast ) ) {
    case C_FUNC_NON_MEMBER:
      ...
      *rv_params_min = is_ambiguous || is_params_unlimited ? 1 : op->params_max;
      *rv_params_max = op->params_max;
      break;
    case C_FUNC_MEMBER:
      if ( !is_params_unlimited ) {
        *rv_params_min = op->params_min;
        *rv_params_max = is_ambiguous ? 1 : op->params_min;
        break;
      }
      FALLTHROUGH;
    case C_FUNC_UNSPECIFIED:
