  ...
  | '(' '^' opt_constvol_list cdecl ')' '(' castlist ')'
    {
      ...
      if (strlen($3) > 0)
        sp = " ";
      $$ = cat($4, $3, ds(sp), ds("block ("), $7, ds(") returning "), NullCP);
      prev = 'b';
    }
  ...
  | opt_constvol_list POINTER TO adecl
    {
      ...
      if (strlen($1) != 0)
        sp = " ";
      $$.left = cat($4.left,ds(op),ds("*"), ds(sp),$1,ds(sp),NullCP);
  ...
  | opt_constvol_list POINTER TO MEMBER OF ClassStruct NAME adecl
    {
      ...
      if (strlen($1) != 0)
        sp = " ";
      $$.left = cat($8.left,ds(op),$7,ds("::*"), ds(sp),$1,ds(sp),NullCP);
